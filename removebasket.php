...
$category=$_SESSION['go'];
  ...
  $basket=$_SESSION['b'];
  $artist=$_SESSION['a'];
  if(($key=array_search($a,$artist)) !==false){
      unset($artist[$key]);
  }
   if(($key=array_search($m,$costs)) !==false){
      unset($costs[$key]);
  }
   if(($key=array_search($n,$basket)) !==false){
      unset($basket[$key]);
  }
...
for($x = 0; $x <$arrlength; $x++) { ?>
       ...
       <tr>
            ...
            <td><a href='removebasket.php?artist=<?php echo $basket[$x]; ?>'>a</a></td>
            </tr>
